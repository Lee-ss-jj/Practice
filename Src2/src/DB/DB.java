...
public class DB {
	...
	public static void DBS() throws SQLException {
		...
		try {
			...
			stmt.execute("grant select, insert, delete, update on 2022지방_1.* identified by '1234'");
		} catch (Exception e) {
			// TODO: handle exception
		}
		stmt.execute("create table area(a_no int primary key not null auto_increment, a_name varchar(2))");
		stmt.execute("create table cafe(c_no varchar(10) primary key, c_name varchar(20), t_no varchar(100), c_tel varchar(15), a_no int, c_address varchar(50), c_price int, constraint foreign key(a_no) references area(a_no))");
		...
		stmt.execute("create table notice(n_no int primary key not null auto_increment, u_no int, n_date date, n_title varchar(20), n_viewcount int, n_open int, constraint foreign key(u_no) references user(u_no))");
		...
		stmt.execute("create table reservation(r_no int primary key not null auto_increment, u_no int, c_no varchar(10), t_no int, r_date date, r_time varchar(10), r_people int, r_attend int, constraint foreign key(u_no) references user(u_no), constraint foreign key(c_no) references cafe(c_no), constraint foreign key(t_no) references theme(t_no))");
		
		String s[] = "area cafe genre map ping user notice quiz theme reservation".split(" ");
